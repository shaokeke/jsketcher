...
function EntityList(props) {
  ...
  return (
    <Field active={active} name={name} onClick={setActive}>
          ...
          value.map((entity, i) => {
            ...
            return (
              ...
              >
                ...
                {!readOnly && (
                  <span className={ls.rm} onClick={() => deselect(entity)}>
                    {" "}
                    <Fa icon="times" />
                  </span>
                )}
...
function EditButton({ model }) {
  ...
  return (
    <span onClick={() => ctx.sketcherService.sketchFace(model)} className={ls.editBtn}>
      <FiEdit />
    </span>
  );
}
