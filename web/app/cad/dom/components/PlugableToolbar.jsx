...
export function ToolbarActionButtons({ actions, showTitles, size }) {
  return actions.map((actionRef, i) => {
    if (actionRef === "-") {
      ...
      return (
        <div key={"ToolbarGroup" + i}>
          <ToolbarGroup>
            <ToolbarActionButtons
              actions={actionRef.slice(0, actionRef.length / 2)}
          ...
          <ToolbarGroup>
            <ToolbarActionButtons
              actions={actionRef.slice(actionRef.length / 2, actionRef.length)}
              showTitles={showTitles}
              size={size}
            />
          </ToolbarGroup>
...
function ActionButton({
  ...
  return (
    <ToolbarButton disabled={!enabled} {...props}>
      {icon}
...
export function ConnectedActionButton(props) {
  ...
  const actionAppearance = useStream(
    (ctx) => ctx.streams.action.appearance[actionId] || NonExistentAppearance(actionId)
  );
